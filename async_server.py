"""
Stable Diffusion 3 Async Server Demo

Demo server for testing the stabilityai/stable-diffusion-3.5-medium model.

https://huggingface.co/stabilityai/stable-diffusion-3.5-medium

https://github.com/huggingface/diffusers/tree/main/examples/server-async
"""
...
async def lifespan(app: FastAPI):  # noqa
    ...
    #                 f"[METRICS] total_requests={total} active_inferences={active}"
    #             )
    ...
    try:
        ...
        try:
            stop_fn = getattr(model_pipeline, "stop", None) or getattr(
                model_pipeline, "close", None
            )
            if callable(stop_fn):
...
app.state: SimpleNamespace  # noqa
...
async def api(json: JSONBodyQueryAPI):
    ...
    initializer_ = app.state.MODEL_INITIALIZER
    ...
    def make_generator():
        g = torch.Generator(device=initializer_.device)
...
async def get_status():
    ...
    return {
        "current_model": server_config.model_path,
...
def parse_args():
    ...
    parser.add_argument(
        "--model_path", type=str, required=True, help="model files path"
    )
    parser.add_argument(
...
if __name__ == "__main__":
    ...
    logger.info(
        f"Pipeline initialized and ready to receive requests (model ={server_config.model_path})"
    )
