...
SERVER_URL = "http://127.0.0.1:8080/api/diffusers/inference"
...
def save_from_url(url: str) -> str:
    ...
    filename = (
        os.path.basename(urllib.parse.urlparse(direct).path)
        or f"img_{int(time.time())}.png"
    )
...
def main():
    ...
    print(
        f"Received {len(urls)} URL(s). Waiting {WAIT_BEFORE_DOWNLOAD}s before downloading..."
    )
